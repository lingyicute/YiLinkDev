...
object Utils {
    ...
    fun tryDecodeBase64(text: String): String? {
        try {
            ...
            Log.i(YILINK_PACKAGE, "Parse base64 standard failed $e")
        }
        try {
            return Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(charset("UTF-8"))
        } catch (e: Exception) {
            Log.i(YILINK_PACKAGE, "Parse base64 url safe failed $e")
        }
        return null
    }
    ...
    fun getUrlContentWithCustomUserAgent(urlStr: String?): String {
        ...
        conn.setRequestProperty("Connection", "close")
        conn.setRequestProperty("User-agent", "YiLink/${BuildConfig.VERSION_NAME}")
        url.userInfo?.let {
