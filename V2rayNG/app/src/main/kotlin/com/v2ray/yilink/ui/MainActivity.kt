            ...
            try {
                ...
                assets.list("")
                        ...
                        ?.forEach {
                            ...
                            Log.i(YILINK_PACKAGE, "Copied from apk assets folder to ${target.absolutePath}")
                        }
            } catch (e: Exception) {
...
//        }
        ...
        R.id.export_all -> {
            if (YilinkConfigManager.shareNonCustomConfigsToClipboard(this, mainViewModel.serverList) == 0) {
...
//        }
    ...
    fun importBatchConfig(server: String?, subid: String = "") {
        ...
        var count = YilinkConfigManager.importBatchConfig(server, subid2, append)
        if (count <= 0) {
            count = YilinkConfigManager.importBatchConfig(Utils.decode(server!!), subid2, append)
        }
        if (count <= 0) {
            count = YilinkConfigManager.appendCustomConfigServer(server, subid2)
        }
        if (count > 0) {
            toast(R.string.toast_success)
            mainViewModel.reloadServerList()
            ...
            Observable.timer(300, TimeUnit.MILLISECONDS)
                    ...
                    .subscribe {
                        try {
                            ...
                            Log.w(YILINK_PACKAGE, e)
                        }
                    }
        } catch (e: Exception) {
            Log.d(YILINK_PACKAGE, e.toString())
        }
    }
