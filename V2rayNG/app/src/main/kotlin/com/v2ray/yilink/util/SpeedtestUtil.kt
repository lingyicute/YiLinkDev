...
import com.v2ray.yilink.extension.responseLength
...
object SpeedtestUtil {
    ...
    fun realPing(config: String): Long {
        return try {
            ...
            Log.d(AppConfig.YILINK_PACKAGE, "realPing: $e")
            -1L
        }
    }
    ...
    fun socketConnectTime(url: String, port: Int): Long {
        try {
            ...
            Log.d(AppConfig.YILINK_PACKAGE, "socketConnectTime IOException: $e")
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return -1
    ...
    fun testConnection(context: Context, port: Int): String {
        ...
        try {
            ...
            // network exception
            Log.d(AppConfig.YILINK_PACKAGE, "testConnection IOException: " + Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } catch (e: Exception) {
            // library exception, eg sumsung
            Log.d(AppConfig.YILINK_PACKAGE, "testConnection Exception: " + Log.getStackTraceString(e))
            result = context.getString(R.string.connection_test_error, e.message)
        } finally {
            conn?.disconnect()
        }
