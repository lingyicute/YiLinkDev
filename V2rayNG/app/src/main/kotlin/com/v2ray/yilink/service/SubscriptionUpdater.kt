...
object SubscriptionUpdater {
    ...
    class UpdateTask(context: Context, params: WorkerParameters) :
        ...
        override suspend fun doWork(): Result {
            Log.d(AppConfig.YILINK_PACKAGE, "subscription automatic update starting")
    ...
    fun importBatchConfig(server: String?, subid: String = "") {
        ...
        val count = YilinkConfigManager.importBatchConfig(server, subid, append)
        if (count <= 0) {
            YilinkConfigManager.importBatchConfig(Utils.decode(server!!), subid, append)
        }
        if (count <= 0) {
            YilinkConfigManager.appendCustomConfigServer(server, subid)
        }
    }
}