...
import com.v2ray.yilink.databinding.ItemRecyclerFooterBinding
import com.v2ray.yilink.databinding.ItemRecyclerMainBinding
...
import com.v2ray.yilink.helper.ItemTouchHelperAdapter
import com.v2ray.yilink.helper.ItemTouchHelperViewHolder
...
//            }
                        ...
                        when (i) {
                            0 -> {
                                if (config.configType == EConfigType.CUSTOM) {
                                    ...
                                    ivBinding.ivQcode.setImageBitmap(YilinkConfigManager.share2QRCode(guid))
                                    AlertDialog.Builder(mActivity).setView(ivBinding.root).show()
                                }
                            }
                            1 -> {
                                if (YilinkConfigManager.share2Clipboard(mActivity, guid) == 0) {
    ...
    private fun shareFullContent(guid: String) {
        if (YilinkConfigManager.shareFullContent2Clipboard(mActivity, guid) == 0) {
            mActivity.toast(R.string.toast_success)
        } else {
            mActivity.toast(R.string.toast_failure)
        }
